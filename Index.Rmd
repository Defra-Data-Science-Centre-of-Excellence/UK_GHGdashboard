...
# Install specific packages (replace with your desired package names)
#packages_to_install <- c("readODS","shinyWidgets")
...
#Define groups of libraries
datalibraries <- c("dplyr", "tidyr", "stringr", "janitor", "readODS", "scales") #data reading and  manipulation
applibraries <- c("shiny", "plotly", "shinyWidgets", "flexdashboard", "DT") #libraries for app

#Require packages
lapply(datalibraries, require, character.only = TRUE)
lapply(applibraries, require, character.only = TRUE)
...
produce_tplot <- shiny::reactive({
  ...
  plot_ly(x= ~year, y= ~Temissions, group= ~Sector, color= ~Sector, 
          type="scatter", mode= "lines", colors = c(
            "#96BC61", #ag green
            "#F0A7CA", #buildings
            "#3CBD9C", #transport
            "#C24841", #electricity
...
Summary Data Tables {data-orientation=rows}
=======================================================================
