...
# downloaded <- file.exists("UKGHG.ods") #checks if file is downloaded in working directory
...
#        
#        "UKGHG.ods", download.file,quiet = TRUE, mode = "wb" )} #else{print('data downloaded')} #name and download or print

downloaded <- file.exists("UKGHG.ods") #checks if file is downloaded in working directory
if(downloaded != T){ #if downloaded is not true
  map2("https://assets.publishing.service.gov.uk/media/67a3117e039d059a1fe5fe9c/final-greenhouse-gas-emissions-2023-by-source-dataset.ods", #update this link when new data available
...
GHG_UK <- read_ods(
  ...
  separate_wider_delim(territorial_emissions_statistics_sector, delim = " - ", #separate category, define delimiter as " - "
  ...
  separate_wider_delim(activity, delim = " - ", #separate activity, define delimiter as " - "
                       ...
                       too_few = "align_start", too_many = "merge", #what to do with fewer or more than 2
                       cols_remove = FALSE) %>% #keep original column
  mutate(department = case_when(territorial_emissions_statistics_sector == "Agriculture" ~ "Defra",
                                territorial_emissions_statistics_sector == "LULUCF" ~ "Defra",
                                territorial_emissions_statistics_sector == "Waste" ~ "Defra",
                                territorial_emissions_statistics_sector == "Buildings and product uses" ~"DESNZ",
                                territorial_emissions_statistics_sector == "Electricity supply" ~"DESNZ",
                                territorial_emissions_statistics_sector == "Fuel supply" ~"DESNZ",
                                territorial_emissions_statistics_sector == "Industry" ~"DESNZ",
                                territorial_emissions_statistics_sector == "Domestic transport" ~"Dft",
                                territorial_emissions_statistics_sector == "International aviation and shipping" ~"Dft"
  )) %>%
  replace(is.na(.), "") #get rid of NA values, input blank

  ##Replace empty source 2 values with source 1
  GHG_UK$source_2[GHG_UK$source_2 == ""] <- NA #replace blanks with NA
  GHG_UK$source_2[is.na(GHG_UK$source_2)] <- GHG_UK$source_1[is.na(GHG_UK$source_2)] 
  
  ##Replace empty source 3 values with source 2
  GHG_UK$source_3[GHG_UK$source_3 == ""] <- NA #replace blanks with NA
  GHG_UK$source_3[is.na(GHG_UK$source_3)] <- GHG_UK$source_2[is.na(GHG_UK$source_3)] 
  
  ##Remove lines which are not included in national totals
  GHG_UK <- subset(GHG_UK, !(included_in_national_total %in% "No"))
...
Data displayed is from the most recent reporting year 2023, previous years can be selected for comparison using the filter below.

### Filters and Further Information
```{r}
...
sliderInput("select_Year", #input name
            ...
            min = 1990, max = 2023, 
            value = c(2023), #initial value, use two numbers to make it possible to select a range
            step = 1,
            sep = "") #remove commas
...
Emissions are reported in megatonnes (Mt) of carbon dioxide (CO2) equivalents (eq.) per year. They refer to domestic emissions only, and exclude emissions which are not accounted for in national totals (International aviation and shipping).
...
produce_eplot <- shiny::reactive({
  ...
  sectors <- GHG_pos %>%
    group_by(territorial_emissions_statistics_sector) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE))
  
  subsectors <- GHG_pos %>%
    group_by(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE))
  
  cat_2 <- GHG_pos %>%
    filter(category_2 != "") %>%
    group_by(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector, category_2) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
    unite('parents', c(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector), remove = TRUE, sep = " - ") #combine higher categories into parents
  
  cat_1 <- GHG_pos %>%
    group_by(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector, category_2, category_1) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
    mutate(parents = case_when(
      category_2 != "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector,category_2, sep = " - "),
      category_2 == "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector, sep = " - ")))
  
  act_1 <- GHG_pos %>%
    group_by(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector, category_2, category_1, activity_1) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
    mutate(parents = case_when(
      category_2 != "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector,category_2, category_1, sep = " - "),
      category_2 == "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector, category_1, sep = " - ")
    ))
  
  act_2 <- GHG_pos %>%
    filter(activity_2 != "") %>%
    group_by(territorial_emissions_statistics_sector, territorial_emissions_statistics_subsector, category_2, category_1, activity_1, activity_2) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
    mutate(parents = case_when(
      category_2 != "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector,category_2, category_1, activity_1, sep = " - "),
      category_2 == "" ~str_c(territorial_emissions_statistics_sector,territorial_emissions_statistics_subsector, category_1, activity_1, sep = " - ")
    ))
  ...
  GHGsb1 <- data.frame(
    parents = c(rep("", root)),
    labels =c(paste0(sectors$territorial_emissions_statistics_sector)),
  ...
  GHGsb2 <- data.frame(
    parents =c(paste0(subsectors$territorial_emissions_statistics_sector), paste0(cat_2$parents), paste0(cat_1$parents), 
               paste0(act_1$parents), paste0(act_2$parents)),
    labels =c(paste0(subsectors$territorial_emissions_statistics_subsector), paste0(cat_2$category_2), paste0(cat_1$category_1), 
...
GHG_neg <- GHG_UK %>%
  ...
  subsectors_neg <- GHG_neg %>%
    group_by(territorial_emissions_statistics_subsector) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE))
  
  cat_neg <- GHG_neg %>%
    group_by(territorial_emissions_statistics_subsector, territorial_emissions_statistics_category) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) 
  
  src_neg <- GHG_neg %>%
    group_by(territorial_emissions_statistics_subsector, territorial_emissions_statistics_category, source_2) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
    unite('parents', c(territorial_emissions_statistics_subsector, territorial_emissions_statistics_category), remove = TRUE, sep = " - ") #combine higher categories into parents
  
  
  ##Now paste together distinct instances into dataframe with hierarchical structure - start simple, set root
  GHGsb1_neg <- data.frame(
    parents =c(rep("", 6)),
    labels =c(paste0(subsectors_neg$territorial_emissions_statistics_subsector)),
  ...
  GHGsb2_neg <- data.frame(
    parents =c(paste0(cat_neg$territorial_emissions_statistics_subsector), paste0(src_neg$parents)),
    labels =c(paste0(cat_neg$territorial_emissions_statistics_category), paste0(src_neg$source_2)),
...
produce_tplot <- shiny::reactive({
  ...
  Sect_time <- GHG_UK %>%
    filter(ghg_grouped %in% input$select_GHG & 
             department %in% input$select_dept) %>%
    group_by(year, territorial_emissions_statistics_sector) %>%
    summarize(Temissions = sum(emissions_mt_co2e, na.rm = FALSE)) %>%
    rename("Sector" = "territorial_emissions_statistics_sector") %>% #rename variable
    ungroup() %>%
...
#Substitute LULUCF with full name
  GHG_UK$territorial_emissions_statistics_sector <- gsub('LULUCF','Land Use, Land Use Change, and Forestry', GHG_UK$territorial_emissions_statistics_sector)
  ...
  GHG_table <- GHG_UK %>%
    ...
    rename("Sector" = "territorial_emissions_statistics_sector") #%>% #rename variable
    #rename("Subsector" = "territorial_emissions_statistics_subsector") #rename variable
  ...
  DT::datatable(GHG_summary, 
                ...
                options = list(
               dom = 'Bfrtip',
               buttons = list(list(extend = "csv", text = "Download Table", filename = "data",
               exportOptions = list(
...
DT::renderDT(server = FALSE,{
  ...
  ag_table <- GHG_UK %>%
    filter(year == input$select_Year &
             ghg_grouped %in% input$select_GHG) %>%
    filter(territorial_emissions_statistics_sector == "Agriculture") %>%
    rename("Subsector" = "territorial_emissions_statistics_subsector") %>% #rename variable
    rename("Category" = "territorial_emissions_statistics_category") %>% #rename variable
...
DT::renderDataTable(server = FALSE,{
  ...
  LULUCF_table <- GHG_UK %>%
    filter(year == input$select_Year &
             ghg_grouped %in% input$select_GHG) %>%
    filter(territorial_emissions_statistics_sector == "LULUCF") %>%
    mutate(pos_e = ifelse(emissions_mt_co2e >= 0, emissions_mt_co2e, 0)) %>%
    mutate(neg_e = ifelse(emissions_mt_co2e <= 0, emissions_mt_co2e, 0)) %>%
    rename("Subsector" = "territorial_emissions_statistics_subsector") %>% #rename variable
    rename("Category" = "territorial_emissions_statistics_category") %>% #rename variable
...
DT::renderDataTable(server = FALSE,{
  ...
  w_table <- GHG_UK %>%
    filter(year == input$select_Year &
             ghg_grouped %in% input$select_GHG) %>%
    filter(territorial_emissions_statistics_sector == "Waste") %>%
    rename("Category" = "territorial_emissions_statistics_category") %>% #rename variable
    rename("Source" = "source_2") #rename variable
...
### Departmental Shares of Emissions

```{r}

produce_deptplot <- shiny::reactive({

#Bar plot by subsector, stacked by GHG with Plotly
deptplot <- GHG_UK %>% 
  filter(year == input$select_Year &
             ghg_grouped %in% input$select_GHG) %>%
  group_by(department, territorial_emissions_statistics_sector) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  ungroup() %>%
  plot_ly(x = ~department, type = 'bar', y = ~Temissions, color = ~territorial_emissions_statistics_sector, colors = colors_alphabetically, 
          marker = list(
            width = 1.5)) %>%
  layout(title = '',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
...
agbar_int <- GHG_UK %>% 
  filter(territorial_emissions_statistics_sector == "Agriculture") %>%
  filter(year == input$select_Year) %>%
  group_by(territorial_emissions_statistics_subsector, ghg, ghg_grouped) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg_grouped, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_subsector, type = 'bar', y = ~CO2, name = 'CO2', marker = list(color ="#3CBD9C")) %>%
...
agcat_int <- GHG_UK %>% 
  ...
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_category, type = 'bar', y = ~Temissions, color = ~territorial_emissions_statistics_subsector, colors = colors_bysize,
          marker = list(
            width = 1.5)) %>%
...
agls_int <- GHG_UK %>% 
  filter(territorial_emissions_statistics_sector == "Agriculture") %>%
  filter(territorial_emissions_statistics_subsector == "Livestock") %>%
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
  ...
  layout(title = 'Agricultural Livestock Emissions by Animal and Category',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'), 
         xaxis = list(title = ''), 
         barmode = 'stack'
         )%>%
...
agls2_int <- GHG_UK %>% 
  ...
  add_trace(y = ~N2O, name = 'N2O', marker = list(color ="#D38744")) %>%
  layout(title = 'Agricultural Livestock Emissions by Animal and GHG',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'), 
         xaxis = list(title = ''), 
         barmode = 'stack'
         )%>%
...
agsoil_int <- GHG_UK %>% 
  filter(territorial_emissions_statistics_sector == "Agriculture") %>%
  filter(territorial_emissions_statistics_subsector == "Agricultural soils") %>%
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
...
agsoil2_int <- GHG_UK %>% 
  ...
  filter(territorial_emissions_statistics_subsector == "Agricultural soils") %>%
  filter(year == input$select_Year) %>%
  group_by(territorial_emissions_statistics_category, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_category, type = 'bar', y = ~CO2, name = 'CO2', marker = list(color ="#3CBD9C")) %>%
  add_trace(y = ~N2O, name = 'N2O', marker = list(color ="#D38744")) %>%
...
agcom_int <- GHG_UK %>% 
  filter(territorial_emissions_statistics_sector == "Agriculture") %>%
  filter(territorial_emissions_statistics_subsector == "Agricultural combustion") %>%
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
...
agother_int <- GHG_UK %>% 
  filter(territorial_emissions_statistics_sector == "Agriculture") %>%
  filter(territorial_emissions_statistics_subsector == "Other agriculture") %>%
  filter(year == input$select_Year) %>%
  group_by(territorial_emissions_statistics_category, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
...
Waste_int <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  group_by(territorial_emissions_statistics_category, ghg, ghg_grouped) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg_grouped, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_category, type = 'bar', y = ~CO2, name = 'CO2', marker = list(color ="#3CBD9C")) %>%
...
produce_wplot2 <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
Waste_int2 <- GHG_UK %>% 
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  group_by(territorial_emissions_statistics_category, activity) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_category, type = 'bar', y = ~Temissions, color = ~activity, colors = colors_bysize
          ) %>%
  layout(title = 'Waste Emissions by TES Category and Activity',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
...
produce_wwplot <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
wwater <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  filter(territorial_emissions_statistics_category == "Wastewater") %>%
  group_by(source, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~source, type = 'bar', y = ~CH4, name = 'CH4', marker = list(color ="#C24841")) %>%
  add_trace(y = ~N2O, name = 'N2O', marker = list(color ="#D38744")) %>%
  layout(title = 'Wastewater Emissions by Source and GHG',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
         barmode = 'stack'
         )%>%
...
produce_incplot <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
incin <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  filter(territorial_emissions_statistics_category == "Incineration without energy recovery") %>%
  ...
  add_trace(y = ~CH4, name = 'CH4', marker = list(color ="#C24841")) %>%
  add_trace(y = ~N2O, name = 'N2O', marker = list(color ="#D38744")) %>%
  layout(title = 'Incineration Emissions by Source and GHG',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
         barmode = 'stack'
         )%>%
...
produce_compostplot <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
compost <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  filter(territorial_emissions_statistics_category == "Composting") %>%
  group_by(source, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~source, type = 'bar', y = ~CH4, name = 'CH4', marker = list(color ="#C24841")) %>%
  add_trace(y = ~N2O, name = 'N2O', marker = list(color ="#D38744")) %>%
  layout(title = 'Composting Emissions by Source and GHG',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
         barmode = 'stack'
         )%>%
...
renderPlotly({produce_compostplot()})
...
produce_ADplot <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
AD <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  filter(territorial_emissions_statistics_category == "Anaerobic digestion") %>%
  group_by(source_1, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~source_1, type = 'bar', y = ~CH4, name = 'CH4', marker = list(color ="#C24841")) %>%
  layout(title = 'Emissions from Anaerobic digestion',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
         barmode = 'stack'
         )%>%
...
produce_fireplot <- shiny::reactive({
  
#Bar plot by category, filled by subsector with Plotly
fire <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(territorial_emissions_statistics_sector == "Waste") %>%
  filter(territorial_emissions_statistics_category == "Accidental fires") %>%
  group_by(source_2, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
  plot_ly(x = ~source_2, type = 'bar', y = ~CH4, name = 'CH4', marker = list(color ="#C24841")) %>%
  layout(title = 'Emissions from Accidental Fires',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
         xaxis = list(title = ''),
         barmode = 'stack'
         )%>%
...
produce_lulucf2 <- shiny::reactive({
  ...
  #Bar plot by category, filled by subsector with Plotly
LULUCFcat_int <- GHG_UK %>% 
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
  filter(territorial_emissions_statistics_sector == "LULUCF") %>%
  group_by(territorial_emissions_statistics_subsector, territorial_emissions_statistics_category) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~territorial_emissions_statistics_subsector, type = 'bar', y = ~Temissions, color = ~territorial_emissions_statistics_category, 
          colors = colors_bysize,
          marker = list(
            width = 1.5
                        )) %>%
...
* Emissions are calculated on an "end user" basis, meaning that emissions are allocated to, for example, a house that receives gas heating rather than the powerplant. You can read more about that in the [Annex 1 document associated with the data](https://assets.publishing.service.gov.uk/media/65ff120af1d3a031c432ac6d/annex-1990-2022-uk-ghg-emissions-final-figures-by-end-user-sector-fuel-uncertainties-estimates.pdf).

* It only includes domestic emissions, and does not include emissions with end users outside the UK. Notably, this means that exported fuels and electricity are excluded, as well as emissions from international aviation.

* It also does not include what are called "embedded emissions" from imported goods, which are all of the emissions associated with production and transport to the UK. Only the transport associated with bringing goods from the country's entry point to the end user are reported here. These have historically accounted for >230 Mt CO2 eq. (peaking at 378 in 2007). They are recorded in a separate ["Carbon footprint for the UK"](https://www.gov.uk/government/statistics/uks-carbon-footprint).

* Emissions are based primarily on modelling calculations rather than direct measurements, and are therefore subject to a margin of error. You can find out more about how the data are calculated and calculations improved in the [National Atmospheric Emissions Inventory (NAEI) reports](https://naei.beis.gov.uk/reports/index).
...
Acknowledgements:
Anne Jones pioneered some visualizations of the UK's GHG Inventory in 2019 and shared valuable insights for this updated visualization. 
Jacob Billingham developed a [Net Zero Inventory Mapping tool](https://jacobbillingham.shinyapps.io/CarbonVisualisations/) in collaboration with Anne Jones, and provided valuable insights, feedback, and coding resources from that project.
Other insightful tools in development include the [Net Zero Systems Tool](https://gateway.zscloud.net/auD?origurl=https%3A%2F%2Fwebapps%2ecbas%2ecloud%2fNZST%2f&_ordtok=7WZ3WVh3BhF6qHkD3HPDHs6mGF) being developed by DESNZ and an Agriculture-specific inventory exploration tool being developed at [ADAS](https://adas.co.uk/).
Devon Barker provided valuable feedback and pointed me to some of the resources used in the development of the dashboard.
Andrew Clark and Harley Stoddart recruited me for this project and provided support throughout its creation.
Useful input and ideas for the tool came from across Defra, and it is my hope that it continues to evolve into ever more useful versions that assist Defra and partners in their analyses and goals as we aim for a Net Zero future.
