...
#Substitute LULUCF with full name
  GHG_UK$tes_sector <- gsub('LULUCF','Land Use, Land Use Change, and Forestry', GHG_UK$tes_sector)
...
deptplot <- GHG_UK %>% 
  filter(year == input$select_Year &
             ghg_grouped %in% input$select_GHG) %>%
  group_by(department, tes_sector) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  ungroup() %>%
  plot_ly(x = ~department, type = 'bar', y = ~Temissions, color = ~tes_sector, colors = colors_alphabetically, 
          marker = list(
            width = 1.5)) %>%
  layout(title = '',
         yaxis = list(title = 'Emissions (Mt CO2 eq.)'),
...
agcat_int <- GHG_UK %>% 
  ...
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~tes_category, type = 'bar', y = ~Temissions, color = ~tes_subsector, colors = colors_bysize,
          marker = list(
            width = 1.5)) %>%
...
Waste_int2 <- GHG_UK %>% 
  filter(year == input$select_Year & ghg_grouped %in% input$select_GHG) %>%
  filter(tes_sector == "Waste") %>%
  group_by(tes_category, activity) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~tes_category, type = 'bar', y = ~Temissions, color = ~activity, colors = colors_bysize
          ) %>%
...
wwater <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(tes_sector == "Waste") %>%
  filter(tes_category == "Wastewater") %>%
  group_by(source, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
...
incin <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(tes_sector == "Waste") %>%
  filter(tes_category == "Incineration without energy recovery") %>%
...
compost <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(tes_sector == "Waste") %>%
  filter(tes_category == "Composting") %>%
  group_by(source, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
...
AD <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(tes_sector == "Waste") %>%
  filter(tes_category == "Anaerobic digestion") %>%
...
fire <- GHG_UK %>% 
  filter(year == input$select_Year) %>%
  filter(tes_sector == "Waste") %>%
  filter(tes_category == "Accidental fires") %>%
  group_by(source_2, ghg) %>%
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  pivot_wider(names_from = ghg, values_from=Temissions) %>%
  droplevels() %>%
...
LULUCFcat_int <- GHG_UK %>% 
  ...
  summarize(Temissions = sum(emissions_mt_co2e, na.rm = TRUE)) %>%
  droplevels() %>%
  plot_ly(x = ~tes_subsector, type = 'bar', y = ~Temissions, color = ~tes_category, 
